def flatten(s):
    ...
    for i in s:
        if type(i) == list:
            initial = initial + flatten(i)
        else:
            initial = sum([[i]], initial)
    return initial 
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(city_a, goal) < distance(city_b, goal):
        return get_name(city_a)
    elif distance(city_a, goal) > distance(city_b, goal):
    ...
    if is_leaf(t1) or is_leaf(t2):
        ...
        for i in range(min(len(branches(t1)), len(branches(t2))), max(len(branches(t1)), len(branches(t2)))):
            if len(branches(t1)) < len(branches(t2)):
                new_branches = new_branches + [branches(t2)[i]]
            else:
                new_branches = new_branches + [branches(t1)[i]]
...
def add_trees2(t1, t2):
    ...
    if not t2:
        return t1
    new_branches1 = branches(t1)
    new_branches2 = branches(t2)
    if len(branches(t1)) > len(branches(t2)):
        e = len(branches(t1)) - len(branches(t2))
        new_branches2 = branches(t2) + [None for _ in range(e)]
    elif len(branches(t1)) < len(branches(t2)):
        f = len(branches(t2)) - len(branches(t1))
        new_branches1 = branches(t1) + [None for _ in range(f)]
    return tree(label(t1)+label(t2), [add_trees2(a, b) for a, b in zip(new_branches1, new_branches2)])
...
def balanced(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    for i in range(len(branches(t))-1):
        a = sum_tree(branches(t)[i+1]) - sum_tree(branches(t)[i])
        if a != 0:
            return False
    for b in branches(t):
        if not balanced(b):
