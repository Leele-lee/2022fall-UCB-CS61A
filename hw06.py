...
class Coin:
    ...
    def worth(self):
        ...
        if Mint.present_year - self.year > 50:
            ...
            return self.cents +(Mint.present_year - self.year)
        # Alternative solutions
        return self.cents + max(0, Miny.present_year - self.year - 50)
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            value1 = self.value + self.last_value 
        new_ins = VirFib(value1) 
        new_ins.last_value = self.value
        return new_ins
    def __repr__(self):
...
class VirFib1():
    ...
    def next(self):
        if self.value == 0:
            ...
            result = VirFib1(self.value + self.previous_value)  
        result.previous_value = self.value  
        return result   
    def __repr__(self):
        return "VirFib object, value " + str(self.value)
...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            ...
            return max(bst_max(t.branches[-1]), t.label)  # when len(t.branches) = 1, t.branches[1] is out index range
    if t.is_leaf():
        return True
    if len(t.branches) == 1:
    ...
    if len(t.branches) > 2:
        return False
    if len(t.branches) == 2:  
        return is_bst(t.branches[0]) and is_bst(t.branches[1]) and bst_min(t.branches[1]) > t.label >=  bst_max(t.branches[0])   
