...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        e = [diff_function(typed_word, word_list[i], limit) for i in range(len(word_list))]
        c = min(e) 
        for i in range(len(word_list)):
            b = diff_function(typed_word, word_list[i], limit)
            if b == c and c <= limit:
                return word_list[i]
        return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if limit < 0:
        return 0
    if typed[c-1:c] != source[c-1:c]:
        return 1 + feline_fixes(typed[:c-1], source[:c-1], limit-1) + len_diff
    else:
        return feline_fixes(typed[:c-1], source[:c-1], limit) + len_diff
...
def minimum_mewtations(start, goal, limit):
    ...
    if (start in goal) or (goal in start):
        return len_diff
        # END
    if start[0:1] == goal[0:1] :  # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    return match(words, [[times_per_player[j][i+1]-times_per_player[j][i] for i in range(len(times_per_player[j])) if i<len(times_per_player[j])-1] for j in range(len(times_per_player))])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    return [[get_word(match, j) for j in word_indices if time(match, i, j) == min([time(match, i, j) for i in player_indices]) and i == min([t for t in player_indices if time(match, t, j) == min([time(match, t, j) for t in player_indices])])] for i in player_indices]
    # END PROBLEM 10
