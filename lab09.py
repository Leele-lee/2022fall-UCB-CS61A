...
def partition_gen(n):
    ...
    def yield_helper(j, k):
        if j == 0:
            ...
            for smallmart in yield_helper(j-k, k) :
                yield [k] + smallmart
    ...
    def vend(self):
        if self.stock <= 0:
            ...
            if self.balance < self.price:
                ...
                b = f'Here is your {self.name}.'
                self.balance = 0
                self.stock -= 1
                return b
            else:
                a = f'Here is your {self.name} and ${self.balance - self.price} change.' 
                self.balance = 0
                self.stock -= 1
                return a
...
def insert(link, value, index):
    ...
    """
    "*** YOUR CODE HERE ***"
    # assert index > len(link), 'Out of bounds!'
    i = 0
    while link is not Link.empty:
        if index == i:
            link.first, link.rest = value, Link(link.first, link.rest)  #不能写成link.rest = link，不然会导致无限循环
            return           #return必不可少，虽然return没有做任何事情，但是它阻止了运行else后面的代码 
        else:
            i += 1
            link = link.rest
    if index >= i:
...
def deep_len(lnk):
    ...
    5
    """
    if lnk is Link.empty:     #能直接对lnk操作的就不对lnk.rest或者lnk.first（同elif）
        return 0
...
def reverse_other(t):
    ...
    n = 0
    while n < i//2:
        t.branches[n].label, t.branches[i-1-n].label = t.branches[i-1-n].label, t.branches[n].label
        n += 1
    for b in t.branches:
        for i in b.branches:
            reverse_other(i)
