...
def balanced(m):
    ...
    if length(left(m))*total_weight(end(left(m))) == length(right(m))*total_weight(end(right(m))):
       if is_mobile(end(left(m))) and is_mobile(end(right(m))):
        ...
        return False
    # Alternative solitions
    if is_planet(m):
        return True
    else:
        left_end = end(left(m)) 
        right_end = end(right(m))
        torque_left = length(left_end) * total_weight(left_end)
        torque_right = length(right_end) * total_weight(right_end)
...
def replace_loki_at_leaf2(t, lokis_replacement):
    if is_leaf(t) and label(t) == 'loki':
        return tree(lokis_replacement)
    else:
        return tree(label(t), [replace_loki_at_leaf2(b, lokis_replacement) for b in branches(t)])
